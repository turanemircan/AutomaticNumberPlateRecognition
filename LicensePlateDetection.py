...
def plate_detection(img):
    ...
    ti_img = cv2.medianBlur(img_gray,5)
    ti_img = cv2.medianBlur(ti_img,5)
    ...
    border = cv2.Canny(ti_img,low,high)
    
    border = cv2.dilate(border,np.ones((3,3),np.uint8),iterations=1)
    
    cnt = cv2.findContours(border,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    cnt = cnt[0]
    ...
    for c in cnt:
        ...
        if(w>h and w>h*2) or (h>w and h>w*2):
            ...
            potantial_plate = img_gray[miny:maxy,minx:maxx].copy()
            potantial_median = np.median(potantial_plate)
            
            control1 = potantial_median > 84 and potantial_median < 200
